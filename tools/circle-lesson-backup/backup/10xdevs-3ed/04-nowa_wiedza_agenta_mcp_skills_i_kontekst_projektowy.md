...
lessonId: 3662388
sectionId: 966234
name: "Agent Onboarding: Agents.md, AI Rules i feedback loops"
---

# Agent Onboarding: Agents.md, AI Rules i pętle feedbacku
...
![](https://images.przeprogramowani.pl/diagrams/lessons-m1-l4-lesson-draft-1-10x.png)
...
![](https://images.przeprogramowani.pl/diagrams/lessons-m1-l4-lesson-draft-2-10x.png)
...
### Hierarcha instrukcji - AGENTS.md oraz CLAUDE.md
...
Społeczność publikuje na GitHubie zbiory system promptów popularnych agentów kodowania. Widać w nich powtarzalny układ: najpierw tożsamość agenta i ograniczenia bezpieczeństwa, potem operacje systemowe, dalej kontekst projektu, reguły wykonywania zadań i dokumentacja narzędzi.

Najbardziej użyteczna nie jest treść pojedynczego promptu, tylko jego anatomia. W przykładach agentów terminalowych osobno widać warstwę bezpieczeństwa, osobno zasady korzystania z narzędzi, a osobno miejsce na kontekst projektu: `CLAUDE.md`, pamięć i instrukcje repozytorium.

Widać też specjalizację. Ogólny model ma instrukcje rozmowy, a agent kodujący ma instrukcje pracy na plikach, terminalu, diffach i testach. To dlatego AGENTS.md nie powinien zastępować system promptu. On ma wypełnić jedną konkretną lukę: lokalną wiedzę o projekcie.

Przyjrzyjmy się konkretnym przykładom. Najlepiej udokumentowanym publicznie przykładem jest [system prompt Claude Code](https://github.com/asgeirtj/system_prompts_leaks/blob/main/Anthropic/claude-code.md) (v2.1.120, ~610 linii).
...
Nie ma tu listy frameworków. Nie ma preferowanego języka. Pierwsza reguła mówi o tym, czego agent robić nie wolno. To celowy sygnał od producenta: środowisko uruchomieniowe, nie styl kodu, definiuje tożsamość agenta.

**Obserwacja 2 — "blast radius" jako operacyjna zasada pracy.** Kilkanaście linii dalej pojawia się zasada, która wymaga od agenta szacowania odwracalności każdej akcji, zanim ją wykona:
...
**Obserwacja 3 — starzenie się pamięci wbudowane w prompt.** W sekcji dotyczącej auto-memory pojawia się rzadko spotykana klauzula epistemiczna:
...
**Obserwacja z Cursora — stan terminala jako plik, nie jako wywołanie narzędzia.** Cursor nie udostępnia agentowi dedykowanego API do sprawdzania terminala. Zamiast tego informacje o uruchomionych procesach i ostatnich komendach są wstrzykiwane pasywnie jako pliki tekstowe w katalogu `terminals/`:
...
Zestawiając oba prompty, widać spójną logikę: **miejsce dla AGENTS.md jest zawsze po warstwie producenta, a przed promptem zadaniowym**. Producent definiuje co agent może, AGENTS.md definiuje jak pracujesz w tym konkretnym projekcie. To nie jest kwestia długości pliku — to kwestia tego, że te dwie warstwy mają różne właścicieli i różne cele.
...
Jeśli zobaczysz błędną decyzję, usuń ją. Jeśli ważna decyzja nie trafiła do pamięci, dopisz ją ręcznie albo przenieś do AGENTS.md, jeśli ma obowiązywać cały zespół.

Reset jest prosty: usuń albo przemianuj katalog `memory/`. Przy kolejnej sesji agent zacznie odbudowywać pamięć.

W Codex desktop app pamięć działa trochę inaczej. Memories są domyślnie wyłączone i według dokumentacji OpenAI na starcie nie są dostępne w Europejskim Obszarze Gospodarczym, Wielkiej Brytanii ani Szwajcarii. Jeśli masz do nich dostęp, włączasz je w ustawieniach Codex albo w `~/.codex/config.toml`:
...
Po włączeniu Codex może przenosić użyteczny kontekst z wcześniejszych wątków do kolejnych: stabilne preferencje, powtarzalne workflow, stack technologiczny, konwencje projektu i znane pułapki. W pojedynczym wątku kontrolujesz użycie pamięci komendą `/memories` — osobno dla tego, czy wątek może używać istniejących memories i czy może posłużyć do wygenerowania nowych.
...
- **On the Impact of AGENTS.md Files on the Efficiency of AI Coding Agents** / Lulla et al. / arXiv 2601.20404 - badanie na OpenAI Codex: około 28% krótszy medianowy czas wykonania i około 16% mniej tokenów wygenerowanych przez agenta w małych PR-ach do 100 linii kodu.
- **Evaluating AGENTS.md: Are Repository-Level Context Files Helpful?** / Gloaguen et al. / ETH Zurich + DeepMind / arXiv 2602.11988 - badanie na Claude Code, Codexie i Qwen Code: redundantne pliki instrukcji podnoszą koszty, a minimalne reguły mają większy sens niż kopia dokumentacji.
- **Cursor Rules documentation** / Cursor / https://cursor.com/docs/rules.md - praktyczne zasady tworzenia reguł: skupione, wykonywalne, ograniczone zakresem, z referencjami do plików zamiast kopiowania dokumentacji.
- **GitHub Copilot Custom Instructions and AGENTS.md Support** / GitHub Changelog - wsparcie dla AGENTS.md i `.github/copilot-instructions.md`.
- **OpenAI Codex documentation - AGENTS.md and Memories** / OpenAI - zasady używania AGENTS.md jako kontekstu projektu oraz `/memories` jako kontroli pamięci w aktualnym wątku.
- **Lost in the Middle: How Language Models Use Long Contexts** / Liu et al. / arXiv 2307.03172 - badanie pokazujące u-shaped attention: modele osiągają najlepszą skuteczność dla informacji na początku i końcu kontekstu, a najgorszą dla treści umieszczonych w jego środku — podstawa dla rekomendacji dotyczących kolejności i długości plików instrukcji.
