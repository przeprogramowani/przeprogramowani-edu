...
// Segment metadata mapping for M5
const SEGMENT_METADATA = {
  'l2-mcp': {
    title_en: 'Model Context Protocol',
    title_pl: 'Model Context Protocol',
    description_en: 'Designing and implementing MCP servers for extending Claude\'s capabilities with custom tools, resources, and data sources',
    description_pl: 'Projektowanie i implementacja serwerów MCP do rozszerzania możliwości Claude o niestandardowe narzędzia, zasoby i źródła danych',
    sort_order: 1
  }
};
...
function parsePromptFiles(directory: string): PromptFile[] {
  ...
  const excludePatterns = [
    ...
    '5x4_prompts.md',
    'README.md',
    'm5-prompt-seed-plan.md',
    'm5-split-prompts-plan.md',
    'm5-extract-plan.md',
    'M5_EXTRACTION_SUMMARY.md'
  ];
...
function generateSeedSQL(pairs: PromptPair[]): string {
  ...
  lines.push('-- =====================================================');
  lines.push('-- 10xDevs M5 (Innovation) Prompts Seed File');
  ...
  lines.push('    -- =====================================================');
  lines.push('    -- 2. Insert/Update Collection: m5-innovation');
  lines.push('    -- =====================================================');
  ...
  lines.push('    ) VALUES (');
  lines.push('        v_org_id,');
  lines.push("        'm5-innovation',");
  lines.push("        'Module 5: Innovation',");
  lines.push("        'Extending AI capabilities through llms.txt documentation, Model Context Protocol (MCP) servers, AI agents in CI/CD pipelines, and model evaluation frameworks for AI-assisted development',");
  lines.push('        5');
  lines.push('    )');
  ...
  lines.push('');
  lines.push("    RAISE NOTICE 'Collection m5-innovation: %', v_collection_id;");
  lines.push('');
  ...
  lines.push('');
  lines.push("    RAISE NOTICE 'Deleted existing prompts for collection m5-innovation';");
  lines.push('');
  ...
  lines.push("    RAISE NOTICE 'Successfully seeded % prompts for M5', " + pairs.length + ";");
  lines.push('END $$;');
  lines.push('');
  lines.push('-- =====================================================');
...
// Main execution
function main() {
  console.log('🚀 Generating M5 Prompts Seed File...\n');

  const inputDir = path.join(__dirname);
  const outputFile = path.join(__dirname, '../../../supabase/seed-prompts-10xdevs-m5.sql');
