...
// Segment metadata mapping (segments only have title, no description)
...
function generateSeedSQL(pairs: PromptPair[]): string {
  ...
  lines.push('-- =====================================================');
  lines.push('-- 10xDevs M3 (Production) Prompts Seed File');
  ...
  lines.push('    -- =====================================================');
  lines.push('    -- 2. Insert/Update Collection: m3-prod');
  lines.push('    -- =====================================================');
  ...
  lines.push('        v_org_id,');
  lines.push("        'm3-prod',");
  lines.push("        'Module 3: Production Ready',");
  lines.push("        'Advanced production-ready development patterns including authentication, testing, refactoring, CI/CD, and deployment strategies',");
  lines.push('        3');
  lines.push('    )');
  ...
  lines.push('');
  lines.push("    RAISE NOTICE 'Collection m3-prod: %', v_collection_id;");
  lines.push('');
  ...
  // Insert prompts
  lines.push('    -- =====================================================');
  lines.push('    -- 4. Insert/Update Prompts');
  lines.push('    -- =====================================================');
  ...
  lines.push("    RAISE NOTICE 'Successfully seeded % prompts for M3', " + pairs.length + ";");
  lines.push('END $$;');
  lines.push('');
  lines.push('-- =====================================================');
...
// Main execution
function main() {
  console.log('🚀 Generating M3 Prompts Seed File...\n');

  const inputDir = path.join(__dirname, 'backup/10xdevs-2ed/prompts/m3/individual');
  const outputFile = path.join(__dirname, 'supabase', 'seed-prompts-10xdevs-m3.sql');
  ...
  Array.from(bySegment.entries())
    ...
    .forEach(([segment, count]) => {
      ...
      console.log(`   ${segment}: ${count} prompts (${meta?.title})`);
    });
