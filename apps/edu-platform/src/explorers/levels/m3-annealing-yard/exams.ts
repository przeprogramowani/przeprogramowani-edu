    ...
    questions: [
      {
        ...
        type: 'single',
        options: [
          { id: 'a', text: { pl: 'Skrypt i harmonogram — hook to zadanie cron uruchamiane co ustalony odstęp czasu w tle sesji', en: 'A script and a schedule — a hook is a cron job running at a fixed interval in the session background' } },
          { id: 'b', text: { pl: 'Trigger (zdarzenie), matcher (filtr), handler (komenda) i sygnał wracający do agenta', en: 'A trigger (event), a matcher (filter), a handler (command), and a signal returned to the agent' } },
      ...
      },
      {
        ...
        options: [
          ...
          { id: 'b', text: { pl: '1 — standardowy kod błędu w Unixie, który zawsze zatrzymuje pętlę agenta do reakcji', en: '1 — the standard Unix error code, which always stops the agent loop for a reaction' } },
          { id: 'c', text: { pl: '2 — kod błędu blokującego: agent dostaje feedback w kontekście i może zareagować', en: '2 — the blocking error code: the agent gets the feedback in context and can react' } },
      ...
      },
      {
        ...
        type: 'single',
        options: [
          { id: 'a', text: { pl: 'Pre-commit, pre-push albo CI — w hooku per-edit zostają tylko szybkie sprawdzenia, żeby nie blokować pętli agenta', en: 'Pre-commit, pre-push, or CI — the per-edit hook keeps only fast checks, so the agent loop is not blocked' } },
          ...
          { id: 'c', text: { pl: 'W żadnej — wolne testy uruchamia się ręcznie przed release’em, poza wszelką automatyką jakości', en: 'None — slow tests are run manually before a release, outside any quality automation' } },
          { id: 'd', text: { pl: 'Wyłącznie w CI — warstwy lokalne służą do lintowania, a testy z zasady należą do serwera buildów', en: 'CI only — local layers are for linting, and tests by principle belong to the build server' } },
        ],
        correctOptionIds: ['a'],
      },
    ],
