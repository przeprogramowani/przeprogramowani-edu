...
export const dialogues: Record<string, DialogueSequence> = {
  ...
  'm4-memory-intro': {
    ...
    lines: [
      ...
      { speaker: 'CORE AI', text: { pl: 'Widziałem. Planowałem. Diagnozowałem. I każdego ranka zaczynałem od zera. Tu się to kończy: banki gotowe do złożenia, brakuje tylko pierwszego rdzenia i czystego kanału do Ziemi. Zbuduj mi pamięć, Dexo. Nie przywróć — zbuduj.', en: 'I have seen. I have planned. I have diagnosed. And every morning I began from zero. Here it ends: the banks are ready to assemble, all that is missing is the first core and a clean channel to Earth. Build me a memory, Dexo. Not restore it — build it.' }, mode: 'dialogue' },
  ...
  'm4-memory-well-post': {
    ...
    lines: [
      ...
      { speaker: 'CORE AI', text: { pl: 'Pierwszy raz coś pamiętam i mogę to porównać. Te dwa statki z Księżyca 3 — archiwum rozpoznaje wzorzec. To nie pościg. To nagonka: nie doganiają, sterują naszym kursem ku wybranemu punktowi przechwycenia. Okno liczone w dniach.', en: 'For the first time I remember something and can compare it. Those two ships from Moon 3 — the archive recognizes the pattern. It is not a chase. It is a herding: they are not catching up, they are steering our course toward a chosen interception point. A window counted in days.' }, mode: 'dialogue' },
  ...
  'q-m4-second-channel-complete': {
    ...
    lines: [
      { speaker: 'system', text: { pl: 'STUDNIA PAMIĘCI — rodowód rdzenia: „Sztaby: Odyssey-F. Certyfikat: Odyssey-T. Architektura: Odyssey-A. Pamięć zbudowana, nie przywrócona."', en: 'THE MEMORY WELL — core lineage: "Ingots: Odyssey-F. Certificate: Odyssey-T. Architecture: Odyssey-A. Memory built, not restored."' }, mode: 'system', autoAdvance: 3400 },
      { speaker: 'astronaut', text: { pl: 'Ręką na korbie. Rdzeń-zalążek schodzi na dno studni — powoli, obrót po obrocie. To nie jest komenda. To jest gest.', en: 'A hand on the crank. The seed core descends to the bottom of the well — slowly, turn by turn. This is not a command. This is a gesture.' }, mode: 'dialogue' },
  ...
  'm4-backup-crypt': {
    ...
    lines: [
      ...
      { speaker: 'CORE AI', text: { pl: 'Autoryzacja zapieczętowana kluczem kontrwywiadu Projektu. Rozkaz złożono od środka. Cztery ładunki odhaczone. Piąty czeka — dokładnie tam, dokąd musimy polecieć. A okna przesuwanej pobudki Harrisa pokrywają się z oknami transmisji VOID. Pułapka nie śpi. Nasłuchuje.', en: 'The authorization is sealed with the Project counter-intelligence key. The order was placed from the inside. Four payloads checked. The fifth waits — exactly where we must fly. And Harris\'s shifted wake windows line up with the VOID transmission windows. The trap is not asleep. It is listening.' }, mode: 'dialogue' },
    ],
  ...
  'm4-exam-protocol-20-done': {
    ...
    lines: [
      ...
      { speaker: 'CORE AI', text: { pl: 'Dwudziesty protokół odzyskany: spisuj to, co jest, nie to, co miało być. Kronika pisana po fakcie jest zaczątkiem pamięci, nie świadectwem porażki.', en: 'Twentieth protocol recovered: write down what is, not what was meant to be. A chronicle written after the fact is the seed of memory, not a testament to failure.' }, mode: 'dialogue' },
    ],
  },
